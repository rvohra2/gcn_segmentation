...
def segmentation_adjacency_T(segmentation, connectivity=8):
    """Generate an adjacency matrix out of a given segmentation."""
    ...
    # Get mass.
    nums, _ = torch.unique(segmentation, return_counts=True)
    n = nums.shape[0]

    # Get adjacency (https://goo.gl/y1xFMq).
    tmp = torch.zeros((n, n), dtype=bool)
...
def create_features(image, segmentation):
    ...
    #print(max_dim)
    if max_dim > config.NUM_FEAT:
        return np.array([])
    else:
        for i in range(len(features)):
            features[i].extend([0] * (config.NUM_FEAT-len(features[i])))
...
def create_target(segmentation, target_mask, num_paths):
    ...
    # for i in range(torch.amax(segmentation)+1):
    #     indices = torch.where(segmentation==i)
    #     patch = target_mask[indices[0], indices[1]]
    #     max_label = (collections.Counter(patch).most_common()[0][0])
    #     y = torch.cat((y,max_label.view(1,1)), dim=1)
    # y = torch.squeeze(y) 
    return y
...
def select_mask_color(cls):
    ...
    instance2_color = [0, 255, 0]
    instance3_color = [0, 0, 255]
...
def build_transforms(is_train):
    if is_train:
        min_size = config.INPUT_HGT
        max_size = config.INPUT_WDT
        crop_prob = 0.0
        flip_prob = 0.5
        transform_list = [
