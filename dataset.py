...
class BaseDataset(Dataset):
    ...
    def _process_one_step(self, image, masks, idx, num_paths):
        ...
        plt.imsave(Path(config.OUTPUT_PATH) / "{:s}_bnd.png".format(self.ids[idx]), seg_img)
        # plt.imshow(seg_img)
        # plt.show()
        ...
        if len(features) == 0:
            ...
            y = Variable(torch.from_numpy(utils.create_target(segmentation, target_mask, num_paths))).type(torch.LongTensor)
    ...
    def __getitem__(self, idx):
        ...
        #         return data
            ...
            #     torch.save(data, out_path)
            #     return data
            data = self._process_one_step(image, out, idx, num_paths=config.OUTPUT_LAYER)
...
class BaseballDataset(BaseDataset):
    ...
    def get_groundtruth(self, idx):
        ...
        # leave only one path
        y_png = cairosvg.svg2png(bytestring=svg_one)
        image = Image.open(io.BytesIO(y_png)).convert("RGB")
        #image = np.asarray(y_img)
